...
require_once t3lib_extMgm::extPath('t3build')
    . 'classes/class.tx_t3build_providerInfo.php';
...
class tx_t3build_module extends t3lib_SCbase
{
    ...
    public function main()
    {
        ...
        if ($GLOBALS['BE_USER']->user['admin']) {
            // Draw the form
            $this->doc->form = '<form action="" method="post" enctype="multipart/'
                . 'form-data">';
            // JavaScript
    ...
    /**
     * Adds some JavaScript to the given string and returns it padded to the length
     * of output_buffering
     *
     * @param string $string The content of the output buffer
     *
     * @return string HTML containing the providers
     */
    public function flush($string)
    {
        static $i = 0;
        $i++;
        $string .= '<a name="jump-' . $i . '"></a><script type="text/javascript">'
            . 'window.location.hash = "jump-' . $i . '";</script>';
        return str_pad(nl2br($string), intval(ini_get('output_buffering')))."\n";
    }

    /**
     * Creates a HTML list with all available providers.
     *
     * @return string HTML for the provider list
     */
    protected function providersAction()
    {
        ...
        foreach ($this->providerInfo->getProviders() as $provider) {
            ...
            $strEncodedLine = htmlspecialchars(trim(implode("\n", $lines[1])));
            $content .= <<<JS
<dt><a href="#" onclick="$('providerInput').value = '$provider'; $$('form')[0].submit();">$provider</a></dt>
<dd style="white-space:pre;">$strEncodedLine</dd>
JS;
        }
        $content .= '</dl>';
    ...
    /**
     * Creates a form with all configuration options for a provider, thus it can be
     * run from backend.
     *
    ...
    protected function configureProviderAction()
    {
        ...
        foreach ($provider->getOptionInfos() as $info) {
            ...
            $title = ucfirst(
                trim(preg_replace('/([A-Z])/', ' $1', $info['property']), ' _')
            );
            preg_match_all('/^\s+\* ([^@\/].*)$/m', $info['comment'], $lines);
            $description = nl2br(trim(implode("\n", $lines[1])));

            $form .= '<dl class="t3buildFormElement"><dt>' . $title . '</dt><dd>'
                . $description . '</dd>';

            switch ($info['type']) {
                case 'boolean':
                case 'bool':
                    $form .= '<dd><input type="checkbox" name="' . $name
                        . '[1]" value="true"';
                ...
                case 'array':
                    foreach ($value as $subValue) {
                        $form .= '<dd><input type="text" name="' . $name
                            . '[]" value="' . $subValue . '"/></dd>';
                    }
                    break;
                case 'string':
                ...
                case 'float':
                default:
                    $form .= '<dd><input type="text" name="' . $name . '[]" value="'
                        . $value . '"/></dd>';
            }
        ...
        $strExtPath = t3lib_extMgm::siteRelPath('t3build');
        $form .= <<<HTM
</div>
...
<script type="text/javascript">
    $$('form')[0].writeAttribute('target', 'consoleOutput')
        .observe(
            'submit',
            function(e) {
...
<div id="consoleOutputContainer" style="display:none;">
    <iframe id="consoleOutput" name="consoleOutput" frameborder="no" src="../{$strExtPath}mod1/empty.html" width="100%" height="100%"></iframe>
</div>
...
HTM;
    ...
    /**
     * Run the provider from configureProviderAction() with the entered options.
     *
     * @param tx_t3build_provider_abstract $provider The provider to run
     * @param array                        $args     The selected options
     *
     * @return void
     */
    protected function runProvider($provider, $args)
    {
        ...
        foreach ($provider->getOptionInfos() as $info) {
            if (substr($info['type'], 0, 4) == 'bool'
                && !isset($args['--'.$info['switch']])
            ) {
        ...
        echo '<body style="color:#fff; background:#000; font:normal 11px Lucida '
            . 'Console, Courier New, serif; white-space:nowrap;">';
...
HTM;
        ...
        foreach ($sections as $sectionName => $section) {
            ...
            $content .= $GLOBALS['TBE_TEMPLATE']->collapseableSection(
                $GLOBALS['LANG']->getLL($sectionName), $sectionContent,
                $sectionName, 't3build.status'
            );
        }
    ...
    /**
     * Checks for the mysql and mysqldump commands on cli
     *
     * @return array Array with the available commands
     */
    ...
    /**
     * Checks if the BE user _cli_t3build exists or creates this user if the command
     * createBeUser is given.
     *
     * @return array An array with the information whether the user exists or not
     */
    protected function getBeUserStatus()
    {
        ...
        if (!$beUser->user['uid']) {
            ...
            $status = $LANG->getLL('notExists')
                . ' <a href="'
                . htmlspecialchars($GLOBALS['MCONF']['_'] . '&CMD=createBeUser') . '">'
                . $LANG->getLL('createBeUser') . '</a>';
            ...
            $status = $LANG->getLL('isAdmin')
                . ' <a href="'.htmlspecialchars($GLOBALS['MCONF']['_']
                . '&CMD=changeBeUser') . '">' . $LANG->getLL('changeBeUser') . '</a>';

            if ($this->CMD == 'changeBeUser') {
                $data = array(
                    'be_users' => array(
                        $beUser->user['uid'] => array(
                            'admin' => 0,
                        ),
                    ),
                );
     ...
     * @param integer Message level (-1 = success (default), 0 = info, 1 = notice,
     *                2 = warning, 3 = error)
     *
     * @return void
     */
