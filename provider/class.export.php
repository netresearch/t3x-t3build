...
class tx_t3build_provider_export extends tx_t3build_provider_abstract
{
    ...
    public function exportAction()
    {
        if (!$this->files && !$this->database) {
            $this->_die(
                'No files and no database? Don\'t have anything more to export!'
            );
        }
    ...
    protected function setup()
    {
        ...
        $this->tempTarFile = $this->tempDir . DIRECTORY_SEPARATOR
            . basename($this->file);
        t3lib_div::mkdir(
            $this->tempInfoDir = $this->tempDir . DIRECTORY_SEPARATOR . '.t3build'
        );

        set_include_path(
            t3lib_extMgm::extPath('t3build') . '/contrib/PEAR' . PATH_SEPARATOR
            . get_include_path()
        );
    ...
    {
        require_once t3lib_extMgm::extPath('lowlevel')
            . 'class.tx_lowlevel_cleaner_core.php';
        require PATH_typo3.'template.php';

        if ($this->cleanCommands == 'all') {
            $availableCommands = array_keys(
                (array) $GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['lowlevel']
                    ['cleanerModules']
            );
            $commands = array_diff(
                $availableCommands, explode(',', $this->skipCleanCommands)
            );
        } else {
    ...
    protected function finish()
    {
        ...
        $this->_debug(
            'Moving archive from ' . $this->tempTarFile . ' to ' . $target
        );
     ...
     * @param string $stripDir A path that should be removed from the archive
     *
     * @return void
     */
    protected function addToTar($file, $stripDir = null)
    {
        if (!$stripDir && strpos($file, $this->tempDir) === 0) {
            ...
            $stripDir = ($stripDir)
                ? t3lib_div::getFileAbsFileName($stripDir) : PATH_site;
        }
    ...
    protected function exportDatabase()
    {
        ...
        $arGlobalIgnoreTables = (array) $GLOBALS['TYPO3_CONF_VARS']['EXTCONF']
            ['t3build']['export']['ignoreTables'];

        foreach ($arGlobalIgnoreTables as $extIgnoreTables) {
        ...
        if (count($ignoreTables)) {
            ...
            foreach ($ignoreTables as $ignoreTable) {
                $pattern = strpos($ignoreTable, '*') !== false
                    ? '/^' . str_replace('*', '.*', $ignoreTable) . '$/' : null;

                foreach ($availableTables as $table) {
                    if ($pattern && preg_match($pattern, $table)
                        || $table == $ignoreTable
                    ) {
        ...
        $fileInfo = $dbInfo->addChild(
            'file', basename($this->tempInfoDir) . '/' . $file
        );
    ...
    protected function exportFiles()
    {
        ...
        foreach ($dirs as $dir) {
            $directory = new RecursiveDirectoryIterator(
                t3lib_div::getFileAbsFileName($dir)
            );
            ...
            foreach ($iterator as $file) {
                ...
                $files[] = (string) $file;
                $infoFile = $infoFiles->addChild(
                    'file', str_replace('\\', '/', substr((string) $file, $l+1))
                );
        ...
        foreach ($files as $i => $file) {
            echo $msg . ': ' . @str_repeat(' ', $strlen - strlen($i + 1)) . ($i + 1)
                . '/' . $count . "\r";
            $this->addToTar($file);
        }
